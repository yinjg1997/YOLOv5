...
def parse_opt(known=False):
    ...
    # 配置模型,--cfg：这个参数是用来选择模型参数的。一般是使用官方的，就是选择models文件夹下那4个文件中的一个。
    # 这4个文件每个特点都不同，具体差异可以去Github上看详细介绍。一般都是使用yolov5s.yaml，因为这个又小又快很适合完成小项目。
    ...
    # 一次对多少数据进行训练分数增大对显卡要求增益了。 --batch-size：每个轮次下图片训练的批次大小。同样对显卡的配置有要求。一般都是默认。
    parser.add_argument("--batch-size", type=int, default=16, help="total batch size for all GPUs, -1 for autobatch")
    # 图片缩放尺寸--imgsz：输入图像的分辨率大小，注意这里是训练和mAP测试的图像尺寸，而不是一个图像的宽高。一般默认。
    parser.add_argument("--imgsz", "--img", "--img-size", type=int, default=640, help="train, val image size (pixels)")
    parser.add_argument("--rect", action="store_true", help="rectangular training")
    # 可以修改这个参数来在以前的模型基础训练上但是需要原来的路径和文件.
    # --resume：这个参数是设置是否从上次一次没训练完的地方继续训练。但要求保留原文件。因为训练过程中会在run文件夹下生成对应的训练文件目录，这个参数要求这个训练文件目录还存在。
    # 例如：default=”runs/train/exp/weights/last.pt”（也可以写绝对路径），不使用的话就填False。
